    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            snowflakes.forEach(flake => {
                ...
                // Efecto de oscilación lateral (wobble)
                const wobbleX = Math.sin(flake.wobble) * 2 * flake.depth;

                // Reaparición en la parte superior
                ...
                if (flake.type === '❄️' || flake.type === '✨') {
                    // Dibujar copo de nieve con forma
                    ctx.fillStyle = `rgba(173, 216, 230, ${flake.opacity})`;
                    ctx.beginPath();

                    // Forma de estrella de nieve
                    for (let i = 0; i < 6; i++) {
                    ...
                    // Otras formas
                    ctx.fillStyle = flake.type === '⭐' ?
                        `rgba(255, 255, 153, ${flake.opacity})` :
                        `rgba(34, 197, 94, ${flake.opacity})`;
            ...
            // Conexiones entre copos cercanos
...
/**
 * ------------------------------------------------------------------
 * 2. COMPONENTES EXISTENTES (manteniendo la funcionalidad)
 * ------------------------------------------------------------------
 */
...
const colorMap = { blue: '#3b82f6', cyan: '#06b6d4', green: '#22c55e', yellow: '#eab308', orange: '#f97316', red: '#ef4444', purple: '#a855f7', pink: '#ec4899', gray: '#6b7280' };
const SkillBadge = ({ name, color }) => {
    const colorHex = colorMap[color] || '#3b82f6';
    return (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#1a1a1a] border border-white/5 hover:border-opacity-50 transition-colors group cursor-default" style={{ '--skill-color': colorHex }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = colorHex + '80'; e.currentTarget.style.backgroundColor = colorHex + '1a'; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.05)'; e.currentTarget.style.backgroundColor = '#1a1a1a'; }}>
            <div className="w-2 h-2 rounded-full" style={{ backgroundColor: colorHex, boxShadow: `0 0 8px ${colorHex}40` }}></div>
            ...
            <TiltCard key={idx} noTilt={true} className="h-full">
                <div className="h-full bg-[#0a0a0a]/80 backdrop-blur-md border border-white/10 rounded-2xl p-5 hover:border-blue-500/30 transition-colors">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {category.skills.map((skill, sIdx) => <SkillBadge key={sIdx} name={skill.name} color={skill.color} />)}
                    </div>
                </div>
            </TiltCard>
        ))}
...
const ProjectCarousel = ({ projects }) => {
    ...
    useEffect(() => {
        const interval = setInterval(() => setCurrentIndex((prev) => (prev + 1) % projects.length), 5000);
        return () => clearInterval(interval);
    }, [projects.length]);

    const handleTouchEnd = () => {
        if (touchStart - touchEnd > 75) setCurrentIndex((prev) => (prev + 1) % projects.length);
        if (touchStart - touchEnd < -75) setCurrentIndex((prev) => (prev - 1 + projects.length) % projects.length);
    };

    return (
        <div className="relative w-full overflow-hidden rounded-xl sm:rounded-2xl border border-white/10 bg-gray-900/50 backdrop-blur-sm shadow-2xl" onTouchStart={e => setTouchStart(e.targetTouches[0].clientX)} onTouchMove={e => setTouchEnd(e.targetTouches[0].clientX)} onTouchEnd={handleTouchEnd}>
            <div className="relative h-56 sm:h-64 md:h-80 lg:h-96 group">
                <img src={projects[currentIndex].image} alt={projects[currentIndex].title} className="w-full h-full object-cover transition-all duration-700 group-hover:scale-105 opacity-60 group-hover:opacity-40" />
                <div className="absolute inset-0 bg-gradient-to-t from-[#050505] via-[#050505]/80 to-transparent flex flex-col justify-end p-4 sm:p-6 md:p-8">
                    ...
                    <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-1 sm:mb-2 leading-tight">{projects[currentIndex].title}</h3>
                    <p className="text-gray-300 text-xs sm:text-sm md:text-base mb-3 sm:mb-6 max-w-lg line-clamp-2 sm:line-clamp-none">{projects[currentIndex].desc}</p>
                    <div className="flex flex-wrap gap-1 sm:gap-2 mb-2 sm:mb-4">
                        {projects[currentIndex].tags.map(tag => <span key={tag} className="text-[8px] sm:text-[10px] uppercase font-bold px-2 sm:px-3 py-1 bg-white/5 text-gray-300 rounded border border-white/10">{tag}</span>)}
                    </div>
                </div>
            </div>
            <div className="absolute bottom-2 sm:bottom-4 left-1/2 -translate-x-1/2 flex gap-4 z-20">
                {projects.map((_, idx) => (
                    <button key={idx} onClick={() => setCurrentIndex(idx)} className={`min-w-[44px] min-h-[44px] flex items-center justify-center`}><span className={`block rounded-full transition-all duration-300 ${idx === currentIndex ? 'bg-blue-500 w-5 h-3' : 'bg-white/50 w-3 h-3 hover:bg-white/70'}`} /></button>
                ))}
            </div>
        </div>
    );
...
const PORTFOLIO_DATA = {
    profile: {
        ...
        subRole: "Fullstack Developer & Automation Engineer",
        avatar: "/profile.png",
        bio: "Apasionado por crear soluciones que conectan el código con el impacto real. Especializado en automatizar procesos, construir sistemas escalables y transformar datos en decisiones.",
        cvLink: "#"
    },
    socials: [
    ...
    stack: [
        ...
        },
        {
            ...
            skills: [
                { name: "Python (intermedio)", color: "yellow" },
                { name: "Node.js", color: "green" },
                { name: "Firebase Firestore", color: "orange" },
                { name: "Supabase", color: "cyan" },
                { name: "SQL Server / ETL", color: "red" },
        ...
        },
        {
            ...
            skills: [
                ...
                { name: "n8n Automation", color: "purple" },
                { name: "Integración de APIs", color: "blue" },
        ...
        },
        {
            ...
            skills: [
                ...
                { name: "VS Code / Cursor", color: "blue" }
            ]
        }
    ],
    projects: [
        {
            title: "Sistema de Gestión de Practicantes — Hospital San José",
            desc: "Sistema desarrollado con Flet y Firebase para registrar practicantes, gestionar sedes, turnos y áreas, validar asistencia con QR y generar reportes automáticos para UADI.",
            image: "https://th.bing.com/th/id/R.e95a4a7c75ef0402c80c60d62fadbfe8?rik=%2bO1HfRAEGKdEUg&pid=ImgRaw&r=0 ",
            tags: ["Flet UI", "Firebase", "Python", "Automatización"]
        },
        {
            title: "Sistema Web de Inventario — SGH Solutions",
            desc: "Sistema de inventarios para registrar productos, movimientos, niveles de stock y dashboards. Incluye alertas automáticas y trazabilidad completa.",
            image: "https://images.unsplash.com/photo-1553413077-190dd305871c?w=800 ",
            tags: ["React", "TailwindCSS", "Supabase", "Node.js"]
        },
        {
            title: "Flujos Automáticos — n8n Workflow Suite",
            desc: "Automatizaciones conectando Google Sheets, Firestore y APIs externas. Reduce tiempos operativos y elimina tareas repetitivas dentro de instituciones.",
            image: "https://images.unsplash.com/photo-1553877522-43269d4ea984?w=800 ",
            tags: ["n8n", "APIs", "Automatización"]
        }
    ]
};
...
/**
 * ------------------------------------------------------------------
 * 4. APP PRINCIPAL CON FONDO NAVIDEÑO
 * ------------------------------------------------------------------
 */
    ...
    return (
        <div className="min-h-screen text-gray-100 font-sans selection:bg-blue-500 selection:text-white overflow-x-hidden pb-12">
            ...
            <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 flex flex-col items-center relative">
                ...
                <div className="absolute bottom-20 right-8 opacity-20 animate-bounce">
                    ...
                    <div className="bg-[#0a0a0a]/80 backdrop-blur-md border border-white/10 rounded-2xl p-4 sm:p-6 md:p-8">
                        <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 mb-6">
                            ...
                            <div className="flex flex-col items-center sm:items-start text-center sm:text-left flex-grow">
                                ...
                                <p className="text-sm text-gray-300 font-semibold mb-1 flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-3 sm:gap-4">
                                    {PORTFOLIO_DATA.socials.map((social, idx) => (
                                        <a key={idx} href={social.link} target="_blank" rel="noopener noreferrer" className="text-white hover:text-blue-400 transition-colors duration-200">
                        ...
                        {/* Contador navideño ficticio */}
                        <div className="grid grid-cols-3 gap-2 sm:gap-4 pt-4 sm:pt-6 border-t border-white/10">
                            <div className="text-center">
                                <p className="text-xl sm:text-2xl md:text-3xl font-bold text-red-500">25</p>
                                <p className="text-[10px] sm:text-xs text-gray-400">Días para Navidad</p>
                            </div>
                            <div className="text-center">
                                <p className="text-xl sm:text-2xl md:text-3xl font-bold text-green-500">12+</p>
                                <p className="text-[10px] sm:text-xs text-gray-400">proyectos creados</p>
                            </div>
                            <div className="text-center">
                                <p className="text-xl sm:text-2xl md:text-3xl font-bold text-blue-500">20+</p>
                                <p className="text-[10px] sm:text-xs text-gray-400">clientes satisfechos</p>
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section className="w-full mb-12">
                    <div className="bg-[#0a0a0a]/60 backdrop-blur-md border border-white/10 rounded-2xl p-6 sm:p-8">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h2 className="text-lg font-bold text-white uppercase tracking-wider">Sobre Mí</h2>
                            <Snowflake size={16} className="text-blue-300 animate-spin ml-auto" />
                        </div>
                ...
                <section className="w-full mb-16">
                    <div className="flex items-center gap-2 mb-6">
                        ...
                        <h2 className="text-lg font-bold text-white uppercase tracking-wider">Herramientas & Stack</h2>
                    </div>
                ...
                <section className="w-full mb-16">
                    <div className="flex items-center gap-2 mb-6">
                        ...
                        <h2 className="text-lg font-bold text-white uppercase tracking-wider">Proyectos Destacados</h2>
                ...
                <section className="w-full mb-12">
                    <TiltCard>
                        <div className="bg-gradient-to-br from-[#111] to-black border border-white/10 rounded-2xl p-8 relative overflow-hidden text-center">
                            ...
                            <div className="inline-flex p-3 rounded-full bg-gradient-to-r from-red-500/20 to-green-500/20 text-white mb-4">
                                <Mail size={24} />
                            </div>
                            ...
                            <p className="text-gray-400 mb-6 max-w-md mx-auto">
                                Estoy disponible para nuevos proyectos de desarrollo, automatización de procesos e integración de soluciones tecnológicas.
                            </p>
                ...
                <footer className="w-full border-t border-white/5 pt-8 mt-8 text-center">
                    <p className="text-sm text-gray-400 italic mb-4 flex items-center justify-center gap-2">
                        <Sparkles size={16} className="text-yellow-400" />
                        "La mejor solución es la que simplifica un problema complejo"
                        <span className="text-red-400"> - BraxDev</span>
                        <Sparkles size={16} className="text-yellow-400" />
                    </p>
                    <div className="mb-4">
                        <p className="font-semibold text-gray-300">© 2025 BraxDev</p>
                        <p className="text-xs text-gray-500 mt-1">🎄 ¡Felices fiestas y próspero año nuevo! 🎅</p>
                    </div>
                    <div className="space-y-1 text-xs text-gray-400">
                        <p>🚀 Combinando código, automatización y datos.</p>
                        <p className="text-gray-500">Creado con ❤️ y espíritu navideño 🎁</p>
                    </div>
                </footer>
